...
Future<dynamic> asyncMap(el, func) async => Future.delayed(Duration(seconds: 1), () => func(el)).catchError((err) {
  print(err);
  return cancellableOperation.cancel();
});

void demoCancellableFuture() async {
  List<dynamic> l = [4, 5, 6, 1, "2", 3, "7", 8, 9];
  List<Future<dynamic>> fl = l.map((el) => asyncMap(el, (x) {
    final r = Future.value(x * x).catchError((e) => cancellableOperation.cancel());
    print(x);
    return r;
  })).toList();
